...
3. **API — [render.com](https://render.com)**: New → Blueprint → connect the
   ...
   **Apply migrations yourself before the API can serve anything useful** —
   the container deliberately does not run `prisma migrate deploy` on boot
   (see the comment in `apps/api/Dockerfile` for why: running the CLI
   inside a locked-down runtime container to lazily fetch its schema-engine
   binary proved unreliable in practice, and auto-migrating on every boot
   is a race condition waiting to happen with more than one instance).
   From your own machine, with `DATABASE_URL` pointed at Neon:
   ```bash
   npx prisma migrate deploy --schema=apps/api/prisma/schema.prisma
   ```
   This repo already includes the initial migration
   (`apps/api/prisma/migrations/`) — run this after every deploy that adds a
   new one.
